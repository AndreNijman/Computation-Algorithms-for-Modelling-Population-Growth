...
def menu():
    ...
    while True:
        ...
        choice = input_validation(
            prompt=colored("Enter your choice: ", "yellow"), type="menu"
        )
...
def run_chosen_module(module_number):
    ...
    if module_number == 1:
        ...
        proj_time = input_validation(
            colored("Enter the amount of time to project into the future: ", "yellow"),
            type="int",
        )
        proj_unit = input_validation(
            colored(
                "Enter the time unit (year, quarter-year month, week, day, half-day, quarter-day, hour, minute, second): ",
                "yellow",
            ),
            type="time_unit",
        )
        ...
        print(
            f"{colored('Sophisticated Model:', 'blue')} I = {s_init}, g = {s_rate}% per {s_unit}, fission event frequency = {s_freq}"
        )
        print(f"{colored('Projection time:', 'cyan')} {proj_time} {proj_unit}\n")

        naive_result = run_models(
            n_init,
            n_rate,
            n_unit,
            n_type,
            projection_time=proj_time,
            projection_time_unit=proj_unit,
        )
        print(
            f'{colored("Naive model projected population size:", "green")} {naive_result}'
        )

        sophisticated_result = run_models(
            s_init,
            s_rate,
            s_unit,
            s_type,
        ...
        print(
            f'{colored("Sophisticated model projected population size:", "blue")} {sophisticated_result}'
        )
        ...
        target = input_validation(
            colored("Enter the target population: ", "yellow"), type="int"
        )
        ...
        print(
            f"{colored('Time taken:', 'cyan')} {fission_count} {fission_unit_label}(s) ({time_elapsed} seconds)"
        )
        ...
        proj_time = input_validation(
            colored("Enter the amount of time to project: ", "yellow"), type="int"
        )
        proj_unit = input_validation(
            colored(
                "Enter the time unit (year, quarter-year, month, week, day, half-day, quarter-day, hour, minute, second): ",
        ...
        target = input_validation(
            colored(
                "Enter the population size to project to (enter 0 to use time instead): ",
                "yellow",
            ),
            type="int",
        )
        ...
        if target == 0:
            proj_time = input_validation(
                colored("Enter the amount of time to project for: ", "yellow"),
                type="int",
            )
            proj_unit = input_validation(
                colored(
                    "Enter the projection time unit (year, quarter-year, month, week, day, half-day, quarter-day, hour, minute, second): ",
                    "yellow",
                ),
                type="time_unit",
            )
        ...
        print(
            tabulate(
                rows,
                headers=[
                    colored("Opening", "yellow"),
                    colored("Added", "yellow"),
                    colored("Closing", "yellow"),
                ],
                tablefmt="grid",
            )
        )
        ...
        for label, fission_seconds in fission_frequencies.items():
            ...
            for i in range(fissions):
                ...
                if label in ["minute", "second"]:
                    if i == 0 or i == fissions - 1:
                        # Record population state for later tabulated display
                        rows.append(
                            [round(population, 2), round(added, 2), round(new_pop, 2)]
                        )
                else:
                    # Record population state for later tabulated display
                    rows.append(
            ...
            print(
                f"{colored('Final population after 1 day:', 'cyan')} {round(population, 4)}"
            )
...
def run_models(
    ...
    if model_type == "naive":
        return initial_population * (
            1 + (growth_rate / 100) * (total_projection_time / growth_unit_time)
        )
...
def input_validation(prompt, type):
    ...
    for _ in range(3):
        ...
        try:
            if type == "menu" and value in valid_menu:
                return int(
                    value
                )  # Return the valid choice directly, no need to call run_chosen_module here
            elif type == "int":
                return int(value)
            elif type == "float":
                return float(value)
    ...
    print(
        colored(
            "Too many invalid attempts. Exiting. AAHAAN STOP SPAMMING, IK ITS YOU 😔😔😔",
            "red",
        )
    )
    exit()
...
def naive_model():
    ...
    init = input_validation(
        colored("Enter the initial population: ", "yellow"), type="int"
    )
    rate = input_validation(
        colored(
            "Enter the growth rate (as a percentage without the symbol): ", "yellow"
        ),
        type="float",
    )
    unit = input_validation(
        colored(
            "Enter the growth time unit (year, quarter-year, month, week, day, half-day, quarter-day, hour, minute, second): ",
            "yellow",
        ),
        type="time_unit",
    )
...
def sophisticated_model():
    ...
    freq = input_validation(
        colored(
            "Enter the fission-event frequency time unit (year, quarter-year, month, week, day, half-day, quarter-day, hour, minute, custom, second): ",
            "yellow",
        ),
        type="time_unit",
    )
    ...
    if freq == "custom":
        freq = input_validation(
            colored(
                "Enter the number of fission events per growth rate time unit: ",
                "yellow",
            ),
            type="float",
        )
